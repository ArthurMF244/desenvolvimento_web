...
async function fetchJson(url, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(payload.erro || `Erro na requisição (${response.status}).`);
  }
...
function renderSidebar() {
  ...
  const initials = profile.nome
    ...
    .join('')
    .toUpperCase();
  const reportsActive = ['indicadores.php', 'relatorios.php'].includes(currentPage());
  ...
  sidebar.innerHTML = `
    <aside class="sidebar">
      ...
      <nav class="menu" aria-label="Menu principal">
        <a class="menu-link ${activeClass(['index.php', 'chamado.php'])}" href="index.php" title="Chamados">
          <i class="fa-solid fa-ticket"></i><span class="menu-label">Chamados</span>
        </a>
        <a class="menu-link ${activeClass(['atribuidos.php'])}" href="atribuidos.php" title="Chamados atribuídos a mim">
          <i class="fa-solid fa-user-check"></i><span class="menu-label">Atribuídos a mim</span>
        </a>
        <a class="menu-link ${activeClass(['meus-chamados.php'])}" href="meus-chamados.php" title="Meus chamados">
          <i class="fa-solid fa-inbox"></i><span class="menu-label">Meus chamados</span>
        </a>
        <a class="menu-link ${activeClass(['usuarios.php'])}" href="usuarios.php" title="Usuários">
        ...
        <div class="menu-group ${reportsActive ? 'open' : ''}">
          ...
          <div class="submenu">
            <a class="${activeClass(['indicadores.php'])}" href="indicadores.php">Indicadores</a>
            <a class="${activeClass(['relatorios.php'])}" href="relatorios.php">Relatórios</a>
          </div>
        </div>
        <a class="menu-link ${activeClass(['configuracoes.php'])}" href="configuracoes.php" title="Configurações">
...
async function loadSystemConfig() {
  try {
    const { data } = await fetchJson(`${API_BASE}/configuracoes.php`);
    if (!data) return;
